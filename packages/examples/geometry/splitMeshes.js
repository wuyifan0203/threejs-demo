import { BufferGeometry, Vector3, Plane, Line3 } from "../../lib/three/three.module";
...
/**
 * @description: 将约等于0的数强制赋值为 0 ，因为精度已经丢失了，这种数字没有精度意义，充满了误差！
 * @param {number} num
 * @return {number}
 */
...
/**
 * @description: 统一数据格式位数，统一处理
 * @param {number} num
 * @return {number}
 */
const fixed = num => Number(num.toFixed(16))
 ...
 * @return {number}
 */
const crossProduct = (v1, v2) => v1.x * v2.y - v2.x * v1.y;




/**
 * @description: 将 BufferGeometry 的position转为三角形
...
/**
 * @description: 判断三角形与平面的关系是否为共面，相离，顶点在面
 * @param {number} a
 * @param {number} b
 * @param {number} c
 * @return {boolean}
...
/**
 * @description: 判断三角形是否为一边相交于平面且一顶点在平面上
 * @param {number} a
 * @param {number} b
 * @param {number} c
 * @return {Array}
...
function checkSegmentOnPlane(a, b, c) {
  if ((a > 0 && b < 0 & c < 0) || (a < 0 && b > 0 & c > 0)) {
    ...
    console.error('三角形精度丢失!，无法继续判断');
  }
}

/**
 * @description: 判断两个乱序线段是否为同一条
 * @param {Vector3[]} l1
 * @param {Vector3[]} l2
 * @return {*}
 */
...
/**
 * @description: 计算点point到平面的距离
 * @param {Vector3} point
 * @param {Vector3} planeNormal
 * @param {Vector3} planePoint
 * @return {number}
 */
...
/**
 * @description: 将当前geometry在平面处进行切割
 * @param {BufferGeometry} geometry
 * @param {Vector3} planeNormal
 * @param {Vector3} planePoint
 * @return {Array<Segment>}
 */
export function isIntersectingPlane(geometry, planeNormal, planePoint) {
  const trangles = getAllTrangles(geometry);
  const intersecting = [];

  /**
   * @description: 负责处理三角形与平面的关系。如果有线段则收集，没有就跳过
   * @param {Vector3} pointA
   * @param {Vector3} pointB
   * @param {Vector3} pointC
   * @return {void}
   */
  function filterTrangle(pointA, pointB, pointC) {
  ...
  for (let i = 0, length = trangles.length; i < length; i = i + 9) {
    const point1 = new Vector3(fixed(trangles[i]), fixed(trangles[i + 1]), fixed(trangles[i + 2]));
    const point2 = new Vector3(fixed(trangles[i + 3]), fixed(trangles[i + 4]), fixed(trangles[i + 5]));
    const point3 = new Vector3(fixed(trangles[i + 6]), fixed(trangles[i + 7]), fixed(trangles[i + 8]));
    filterTrangle(point1, point2, point3);
  }
  return intersecting
}

/**
 * @description: 将回路线段分割开，返回所有回路
 * @param {Array<[Vector3,Vector3]>} segment
 * @return {Array<Loop>}
 */
export function sortSegmentToLoop(segment) {
  const result = [[]];
 ...
 * @return {{enable:boolean,order:number}}
 */
...
/**
 * @description: 寻找可排序的线段索引
 * @param {Segment} currentSegment
 * @param {Array<Segment>} remainingSegments
 * @return {{index:number,order:number}}
 */
...
/**
 * @description: 将线段格式化，返回要求的格式
 * @param {Array<Loop>} loops
 * @return {{body:Array<number>,holes:Array<number[]>}}
 */
export function formateLoops(loops) {

  /**
   * @description: 得到面积最大的索引
   * @param {number[]} areas
   * @return {number}
   */
  ...
  function getLoopArea(loop) {
    ...
    for (let index = 0, length = loop.length; index < length; index++) {
  ...
  /**
   * @description: 将回路转成对应方向的路径
   * @param {Loop} loop
   * @param {"AC" | "C"} direction
   * @return {number[]}
   */
