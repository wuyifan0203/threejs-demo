...
function init() {
  ...
  renderer.clearStencil(); // 清除 stencil 缓冲区
  renderer.autoClearStencil = false; // 禁止自动清除 stencil 缓冲区
  ...
  scene.add(new AmbientLight());
    ...
    // Front Face Object
    const Icosahedron = new IcosahedronGeometry(1, 8);
    const IcosahedronMaterial = new MeshPhongMaterial({
      color: 0xff0000,
      stencilWrite: true,
      stencilRef: 1,
      stencilFunc: EqualStencilFunc,
    });
    const IcosahedronMesh = new Mesh(Icosahedron, IcosahedronMaterial);
    IcosahedronMesh.position.set(0, 0, -1);
    IcosahedronMesh.scale.set(0.5, 0.5, 0.5);
    IcosahedronMesh.renderOrder = 2;
