 ...
 * @LastEditTime: 2025-05-23 01:07:44
...
class OpenCascadeHelper {
  ...
  #tessellate(shape) {
    ...
    for (
      ExpFace.Init(shape, this.occ.TopAbs_FACE, this.occ.TopAbs_SHAPE);
      ExpFace.More();
      ExpFace.Next()
    ) {
      ...
      // write normal buffer
      const myNormal = new this.occ.TColgp_Array1OfDir(
        1,
  ...
  convertBuffer(shape) {
    ...
    const triangleCount = faceList.reduce(
  ...
  convertShape(shapes, lineDeviation = 0.1, angleDeviation = 0.5) {
    ...
    try {
      ...
      new this.occ.BRepMesh_IncrementalMesh(
        shape,
        lineDeviation,
        false,
        angleDeviation
      ...
      this.forEachFace(shape, (face) => {
        ...
        const handelTriangulation = this.occ.BRep_Tool.prototype.Triangulation(
          face,
          location
        );
        ...
        this.forEachEdge(face, (edge) => {
          ...
          if (totalEdgeHashes2.hasOwnProperty(edgeHash)) {
            ...
            const handlePolygonTra =
              this.occ.BRep_Tool.prototype.PolygonOnTriangulation(
                edge,
                handelTriangulation,
                location
              );
            ...
            for (
              let i = 0, l = edgeNodes.Length(), j = i;
              i < l;
              i++, j = i * 3
            ) {
      ...
      for (let i = 0; i < uvBoxes.length; i++) {
        ...
        for (
          let j = 0, k = 0, l = faceInfo.uv.length / 2;
          j < l;
          j++, k = j * 2
        ) {
          ...
          _v1.x =
            (_v1.x * (box.w - padding) + (box.x + padding / 2)) /
            Math.max(packing.w, packing.h);
          _v1.y =
            (_v1.y * (box.h - padding) + (box.y + padding / 2)) /
            Math.max(packing.w, packing.h);
      ...
      this.forEachEdge(shape, (edge) => {
        ...
        if (!totalEdgeHashes2.hasOwnProperty(edgeHash)) {
          ...
          const deflection = new this.occ.GCPnts_TangentialDeflection(
            adaptorCurve,
            lineDeviation,
            0.1
          );
          ...
          for (
            let i = 0, l = deflection.NbPoints(), j = 0;
            i < l;
            i++, j = i * 3
          ) {
            const point = deflection
              .Value(i + 1)
              .Transformed(location.Transformation());
  ...
  static convertBufferAttribute(faceList, edgeList) {
    const position = [];
    const normal = [];
    const indices = [];
    const uvs = [];
