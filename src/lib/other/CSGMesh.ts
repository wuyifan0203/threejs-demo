...
import {BufferGeometry,Vector3,Matrix3,Mesh,Matrix4,Material,BufferAttribute} from 'three'
...
class CSG {
    ...
    private static ttvv0 = new Vector3()
    private static tmpm3 = new Matrix3()

    static fromMesh = function (mesh:Mesh, objectIndex?: object) {
    ...
    static toMesh = function (
        ...
        geom.setAttribute(
            'position',
            new BufferAttribute(vertices.array, 3)
        )
        ...
        colors &&
            geom.setAttribute(
                'color',
                new BufferAttribute(colors.array, 3)
            )
        if (grps.length) {
            let index: any[] = []
            let gbase = 0
