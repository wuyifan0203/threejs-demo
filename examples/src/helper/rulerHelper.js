...
function init() {
  ...
  ruler2Folder.add(controls.ruler2, 'unit', 0.1, 600, 0.01).onChange(() => update());

  const ruler3Folder = gui.addFolder('Ruler3');
  ruler3Folder.add(controls.ruler3, 'zeroX', -900, 1800, 10).name('zeroX position').onChange(() => update2());
  ruler3Folder.add(controls.ruler3, 'zeroY', -900, 1800, 10).name('zeroY position').onChange(() => update2());
  ruler3Folder.add(controls.ruler3, 'unit', 0.1, 600, 0.01).onChange(() => update2());
  ...
  function createRuler2(width, height) {
    ...
    const { zero } = controls.ruler2;
    let { unit } = controls.ruler2;
    // 计算量
  ...
  function createRuler3(width, height) {
    ...
    ctx.beginPath();
    for (let i = -offsetY, j = 0; i < lengthY; i++, j++) {
